...
# MODULES (import modules into agent, connect to buffers, and add initial content)
    ...
    def dominus_get_code(b_context='status:unoccupied'): 
        b_method.set('method:get_code target:response content:lectus_1 state:start')
        b_context.set('status:get_code')
        print 'getting the code'

    def dominus_response1(b_method='state:finished',
                          b_context='status:get_code'):
        b_method.set('method:response target:response content:dominus_1a state:start')
        print 'entering the code'
        b_context.set('status:response')

    def dominus_finished(b_method='state:finished',  ## this line assumes waiting for the last method above to finish
                         b_context='status:response'):
        print 'planning unit initiated'
 #       b_plan_unit.modify(planning_unit='structura',state='begin_sequence', ptype='ordered')
        b_plan_unit.modify(planning_unit='structura',cuelag='none',cue='start',unit_task='primus',state='begin_sequence',ptype='ordered')

        b_context.set('status:occupied')
       
##    def structura_planning_unit(b_context='status:unoccupied',
##                          display='state:structura'): 
##        b_plan_unit.modify(planning_unit='structura',cuelag='none',cue='start',unit_task='primus',state='begin_sequence',ptype='ordered')
##        b_context.set('status:occupied')# update context status to occupied
